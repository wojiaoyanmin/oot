...
def collect_files(Images_dir, Human_dir ):
    files = []
    suffix='jpg'
    for Image_file in glob.glob(osp.join(Images_dir, '*.jpg')):
        Human_file = osp.join(Human_dir, osp.basename(Image_file))[:-len(suffix)]+'png'
        files.append((Image_file, Human_file))
...
def load_img_info(files):
    Image_file, Human_file = files
    Human_img = mmcv.imread(Human_file, 'unchanged')
    ids = np.unique(Human_img)
    anno_info = []
    for id in ids:
        if id == 0:
            continue
        category_id = 0  # 把background作为最后一个类别
        iscrowd = 0
        mask = np.asarray(Human_img == id, dtype=np.uint8, order='F')
    ...
    info = dict(
        ...
        height=Human_img.shape[0],
        width=Human_img.shape[1],
        anno_info=anno_info,
        segm_file=None)
    return info
    ...
    for split, json_name in set_name.items():
        ...
        with mmcv.Timer(
            ...
            files = collect_files(
                ...
                osp.join(cihp_path, split, args.Human_dir))
