...
class SOLOHead(nn.Module):
    ...
    def get_seg_single(self,
        ...
        for i in range(num_humans):
            ...
            #     print(part_score[i])
            #     plt.show()
            cur_score_init = torch.zeros([seg_masks.shape[-2] , seg_masks.shape[-1]], dtype=torch.float, device=seg_masks.device)
            ...
            for j in range(keep.sum()):
                ...
                cur_score_init[part_mask[j]]=part_score[j]
            
            final_mask = final_mask * human_seg_masks[i]
            cur_score = cur_score_init * human_seg_masks[i]
