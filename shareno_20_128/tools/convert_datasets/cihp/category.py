...
def collect_files(Images_dir, Categories_dir ):
    files = []
    suffix='jpg'
    for Image_file in glob.glob(osp.join(Images_dir, '*.jpg')):
        Categories_file = osp.join(Categories_dir, osp.basename(Image_file))[:-len(suffix)]+'png'
        files.append((Image_file, Categories_file))
...
def load_img_info(files):
    Image_file, Categories_file = files
    Categories_img = mmcv.imread(Categories_file, 'unchanged')
    ids = np.unique(Categories_img)
    anno_info = []
    for id in ids:
        if id == 0:
            continue
        category_id = int(id) - 1  # 把background作为最后一个类别
        iscrowd = 0
        mask = np.asarray(Categories_img == id, dtype=np.uint8, order='F')
    ...
    info = dict(
        ...
        height=Categories_img.shape[0],
        width=Categories_img.shape[1],
        anno_info=anno_info,
        segm_file=None)
    return info
...
def parse_args():
    ...
    parser.add_argument('--Category-dir', default='Category_ids', type=str)
...
def main():
    ...
    set_name = dict(
        train='Categories_train.json',
        val='Categories_val.json')
    ...
    for split, json_name in set_name.items():
        ...
        with mmcv.Timer(
            ...
            files = collect_files(
                ...
                osp.join(cihp_path, split, args.Category_dir))
