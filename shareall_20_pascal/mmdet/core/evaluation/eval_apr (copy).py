...
def convert2evalformatbit(inst_id_map, id_to_convert=None):
    """
    param:
        inst_id_map:[h, w]
        id_to_convert: a set
    ...
    for i in id_to_convert:
        masks.append(inst_id_map[i])

    return masks, len(masks)

def compute_class_ap(apr_results, image_id_list, class_id, iou_threshold,INST_PART_GT_DIR,PREDICT_DIR, APR_CLASSES):
    ...
    for image_id in image_id_list:
        ...
        pre_img = pickle.load(gzip.open(apr_results[image_id]))
        pre_img = np.array(pre_img.cpu())
        ...
        pre_mask, n_pre_inst = convert2evalformatbit(pre_img, set(pre_id))
