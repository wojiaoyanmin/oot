...
from mmdet.core.evaluation import iou_eval
...
def get_gt(list_dat, task_id=None):
    ...
    for dat in tqdm(list_dat, desc='Loading gt..'):#num picture
        ...
        if len(dat['bboxes']) == 0:
            ...
            gt_box = []
            anno_adds = []
            for bbox in dat['bboxes']:#single picure num det
...
def eval_seg_ap(results_all, dat_list, nb_class=59, ovthresh_seg=0.5, Sparse=False, From_pkl=False, task_id=None):
    ...
    for imagename in tqdm(results_all.keys(), desc='Loading results ..'):#num picture
        if From_pkl:
        ...
        if len(det_rects)==0:
            continue
        for idx, rect in enumerate(det_rects):#single picture num human pred
            image_ids.append(imagename)
            confidence.append(rect[-1])
            BB.append(rect[:4])
            Local_segs_ptr.append(idx)
    confidence = np.array(confidence)#num_picture*num_human_pred
    BB = np.array(BB)
    ...
    nd = len(image_ids)#num picture
    tp_seg = np.zeros(nd)
    fp_seg = np.zeros(nd)
    pcp_list = []
    for d in trange(nd, desc='Finding AP^P at thres %f..' % ovthresh_seg):  # trange进度条
        ...
        for i in range(len(R['anno_adds'])):#pred picuture de suo suo you de det label
            ...
            # print('mask_gt',np.unique(mask_gt))
            # print(mean_seg_iou)
            # print(seg_iou)
            # plt.subplot(1,2,1)
        ...
        if ovmax > ovthresh_seg:
            if not R['det'][jmax]:
                tp_seg[d] = 1.
                R['det'][jmax] = 1
                pcp_d = len(mask_gt_u[np.logical_and(mask_gt_u > 0, mask_gt_u < nb_class)])#label de lei bie shu
                pcp_n = float(np.sum(seg_iou_max[1:] > ovthresh_seg))#[1:]chu qu beijing 
                if pcp_d > 0:
    ...
    print('threshold,AP_seg, PCP:', ovthresh_seg, ap_seg, pcp)
    return {'AP_seg':ap_seg, 'PCP':pcp}
...
def get_prediction_from_gt(dat_list, NUM_CLASSES, cache_pkl=False, cache_pkl_path='tmp/', Sparse=False):
    ...
    for dat in tqdm(dat_list, desc='Generating predictions ..'):
        ...
        for bbox in dat['bboxes']:
            mask_gt = np.array(Image.open(bbox['ann_path']))
            if len(mask_gt.shape) == 3: mask_gt = mask_gt[:, :, 0]  # Make sure ann is a two dimensional np array.
            if np.sum(mask_gt) == 0: continue  # 有数值的，记得
